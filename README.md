...
- **[review-queue](skills/review-queue/SKILL.md)** — answers "what do I have to review here?" and then gets out of the way: it lists the open PRs requesting your review, newest first, and opens one space per PR running `local-review`, so each review arrives on its own instead of stacking up in the session you asked from. A space is a devspaces workspace in the `Local Review` group when it runs in a pod, and a background session anywhere else, and there is no falling from one to the other: a path that does not work stops and says so. Re-running it opens only what is new and names the spaces whose PR no longer needs you, for you to delete. It never reviews a PR itself, which is the point: reviews written in one context are written in the shadow of each other.
