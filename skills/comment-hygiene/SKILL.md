...
4. Any comment five lines or longer. No comment that long is essential the whole way through, so keep only the lines that carry knowledge from the list above, and refactor away or drop the rest.
...
Length is a red flag by itself. Suspect a comment of three lines, and suspect one of four lines more: assume it can be said in less, and leave it at that length only when every line is essential, which is rare. Shrinking a comment uses the same four fixes as removing one, so move whatever the code can say into the code and keep what is left. Four things usually make a comment long, and each of them is worth cutting: context the reader does not need, a restatement of the code sitting next to the part that was worth saying, a point made in a verbose way instead of a short one, and a pointer to code elsewhere, which goes stale as soon as that code changes.
